...
function Homepage(props) {
  ...
  useEffect(() => {
    ...
    if (!showList.length) {
      if (isLogin) {
        ...
        fetchData(getUnLoginList, {
          params: { per_page: 12, page: (page + 2) / 2 },
        });
      }
    } else {
  ...
  const fetchData = async (fn, params) => {
    try {
      ...
      let showList = [...webinarsAllList, ...resList].slice(
        page * 6,
        page * 6 + 6
      );
      setWebinarsList(showList);
  ...
  const registryHandler = () => {
    if (isLogin) {
      window.scrollTo({ top: 1071, behavior: "smooth" });
    } else {
      navigate("/login");
    }
  };
    ...
    <div>
      ...
      <div className="content-informations">
        <div className={page ? "icon-class" : "no-icon-class"}>
          {page ? (
            <span className="material-icons" onClick={() => setPage(page - 1)}>
              arrow_back
            </span>
          ) : null}
        </div>
          ...
          {webinarsList.map((card, index) => {
            return (
              <div className="card-item" key={`card_${index}`}>
                <div className="card-item-details-date">
                  {card.created_at.split(" ")[0]}
                </div>
                <div className="card-item-details-topic">{card.title}</div>
                <div className="card-item-details-contents">{card.content}</div>
                <div className="card-item-details-time">
                  {card.cardFormateTime}
                </div>
                <div className="card-item-details-bottom">
                  <div
                    className="card-item-details-bottom-registry"
                    onClick={registryHandler}
                  >
                    Register Now
                  </div>
                  <div className="card-item-details-bottom-icon">
                    <span className="material-icons icon-setting">
                      double_arrow
                    </span>
                  </div>
                </div>
        ...
        <div className={!isLogin ? "icon-class" : "no-icon-class"}>
          {isLogin && webinarsAllList.length / 6 > page + 1 ? (
            <span className="material-icons" onClick={() => setPage(page + 1)}>
              arrow_forward
            </span>
          ) : null}
